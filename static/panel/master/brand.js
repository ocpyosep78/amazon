...
Ext.onReady(function() {
	...
	var main_store = Ext.create('Ext.data.Store', {
		...
		proxy: {
			type: 'ajax',
			url : URLS.base + 'panel/master/brand/grid', actionMethods: { read: 'POST' },
	...
	var main_grid = new Ext.grid.GridPanel({
		...
		columns: [ {
			...
			}, {	header: 'Alias', dataIndex: 'alias', sortable: true, filter: true, width: 200, flex: 1
		} ],
		tbar: [ {
		...
		update: function(Param) {
			...
			Ext.Ajax.request({
				url: URLS.base + 'panel/master/brand/action',
	...
	function main_win(param) {
		var win = new Ext.Window({
			layout: 'fit', width: 390, height: 128,
			...
			listeners: {
				show: function(w) {
					var Title = (param.id == 0) ? 'Entry Brand - [New]' : 'Entry Brand - [Edit]';
					w.setTitle(Title);
					
					Ext.Ajax.request({
						url: URLS.base + 'panel/master/brand/view',
						success: function(Result) {
			...
			save: function() {
				...
				Func.ajax({ param: ajax, url: URLS.base + 'panel/master/brand/action', callback: function(result) {
					Ext.Msg.alert('Informasi', result.message);
					if (result.status) {
						main_store.load();
						win.hide();
