...
Ext.onReady(function() {
	...
	var main_store = Ext.create('Ext.data.Store', {
		...
		fields: [ 'id', 'name', 'store', 'link', 'library', 'is_active', 'category_sub_id', 'category_sub_name', 'category_id', 'category_name' ],
		proxy: {
			type: 'ajax',
	...
	var main_grid = new Ext.grid.GridPanel({
		...
		columns: [ {
			...
			}, {	header: 'Store', dataIndex: 'store', sortable: true, filter: true, width: 200, flex: 1
			}, {	header: 'Category', dataIndex: 'category_name', sortable: true, filter: true, width: 200
			}, {	header: 'Sub Category', dataIndex: 'category_sub_name', sortable: true, filter: true, width: 200
	...
	function main_win(param) {
		var win = new Ext.Window({
				...
				show: function(w) {
					...
					Ext.Ajax.request({
						...
						success: function(Result) {
							...
							win.store = new Ext.form.TextField({ renderTo: 'storeED', width: 225, allowBlank: false, blankText: 'Masukkan Store' });
							win.link = new Ext.form.TextField({ renderTo: 'linkED', width: 225, allowBlank: false, blankText: 'Masukkan Link' });
							win.category = Combo.Class.Category({
								renderTo: 'categoryED', width: 225, allowBlank: false, blankText: 'Masukkan Category', listeners: {
							...
							win.library = new Ext.form.TextField({ renderTo: 'libraryED', width: 225, allowBlank: false, blankText: 'Masukkan Library' });
			...
			save: function() {
				...
				if (! win.store.validate()) {
					is_valid = false;
				}
				if (! win.link.validate()) {
					is_valid = false;
				}
				if (! win.library.validate()) {
					is_valid = false;
				}
