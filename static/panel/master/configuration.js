...
Ext.onReady(function() {
	...
	var main_store = Ext.create('Ext.data.Store', {
		...
		fields: [ 'id', 'name', 'desc', 'value' ],
		proxy: {
			type: 'ajax',
			url : URLS.base + 'panel/master/configuration/grid', actionMethods: { read: 'POST' },
	...
	var main_grid = new Ext.grid.GridPanel({
		...
		columns: [ {
					header: 'Title', dataIndex: 'name', sortable: true, filter: true, width: 200
			}, {	header: 'Description', dataIndex: 'desc', sortable: true, filter: true, width: 200, flex: 1
			}, {	header: 'Value', dataIndex: 'value', sortable: true, filter: true, width: 100
		} ],
		tbar: [ {
		...
		update: function(Param) {
			...
			Ext.Ajax.request({
				url: URLS.base + 'panel/master/configuration/action',
	...
	function main_win(param) {
		var win = new Ext.Window({
			layout: 'fit', width: 390, height: 163,
			...
			listeners: {
				show: function(w) {
					var Title = (param.id == 0) ? 'Entry Configuration - [New]' : 'Entry Configuration - [Edit]';
					w.setTitle(Title);
					
					Ext.Ajax.request({
						url: URLS.base + 'panel/master/configuration/view',
						success: function(Result) {
							w.body.dom.innerHTML = Result.responseText;
							
							win.id = param.id;
							win.name = new Ext.form.TextField({ renderTo: 'nameED', width: 225, allowBlank: false, blankText: 'Masukkan Title' });
							win.desc = new Ext.form.TextField({ renderTo: 'descED', width: 225, allowBlank: false, blankText: 'Masukkan Description' });
							win.value = new Ext.form.TextField({ renderTo: 'valueED', width: 225, allowBlank: false, blankText: 'Masukkan Value' });
							...
							if (param.id > 0) {
								...
								win.value.setValue(param.value);
							}
						}
					});
				},
			...
			save: function() {
				...
				Func.ajax({ param: ajax, url: URLS.base + 'panel/master/configuration/action', callback: function(result) {
					Ext.Msg.alert('Informasi', result.message);
					if (result.status) {
						main_store.load();
						win.hide();
