...
Ext.onReady(function() {
	...
	var main_store = Ext.create('Ext.data.Store', {
		...
		proxy: {
			type: 'ajax',
			url : URLS.base + 'panel/user/subscribe/grid', actionMethods: { read: 'POST' },
	...
	var main_grid = new Ext.grid.GridPanel({
		...
		columns: [ {
					header: 'Email', dataIndex: 'email', sortable: true, filter: true, width: 200, flex: 1
					...
					items: [ {
						...
						handler: function(grid, rowIndex, colIndex) {
							...
							Func.ajax({ param: param, url: URLS.base + 'panel/user/subscribe/action', callback: function(result) {
								if (result.status) {
									grid.store.load();
								}
							} });
		...
		update: function(Param) {
			...
			Ext.Ajax.request({
				url: URLS.base + 'panel/user/subscribe/action',
	...
	function main_win(param) {
		var win = new Ext.Window({
			layout: 'fit', width: 390, height: 103,
			...
			listeners: {
				show: function(w) {
					var Title = (param.id == 0) ? 'Entry Subscribe - [New]' : 'Entry Subscribe - [Edit]';
					w.setTitle(Title);
					
					Ext.Ajax.request({
						url: URLS.base + 'panel/user/subscribe/view',
						success: function(Result) {
			...
			save: function() {
				...
				Func.ajax({ param: ajax, url: URLS.base + 'panel/user/subscribe/action', callback: function(result) {
					Ext.Msg.alert('Informasi', result.message);
					if (result.status) {
						main_store.load();
						win.hide();
